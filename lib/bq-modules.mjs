...
/* ---------- Call tracking mart ---------- */
async function aggCalls(bq, ds, s, e) {
  const [r = {}] = await q(bq,
    "SELECT SUM(calls) AS calls, SUM(answered) AS answered, SUM(missed) AS missed, SUM(voicemail) AS voicemail, " +
    "SUM(qualified) AS qualified, SUM(first_time_callers) AS first_time, SUM(duration_seconds) AS duration, " +
    "SUM(talk_seconds) AS talk, SUM(value) AS value " +
    "FROM `" + ds + ".fact_calls_daily` WHERE date BETWEEN @s AND @e", { s, e });
  return {
    calls: n(r.calls), answered: n(r.answered), missed: n(r.missed), voicemail: n(r.voicemail),
    qualified: n(r.qualified), firstTime: n(r.first_time), duration: n(r.duration),
    talk: n(r.talk), value: n(r.value),
  };
}

async function callTracking(bq, ds, w) {
  const [cur, prev] = await Promise.all([aggCalls(bq, ds, w.start, w.end), aggCalls(bq, ds, w.prevStart, w.prevEnd)]);
  const [srcRows, recent] = await Promise.all([
    q(bq, "SELECT source, medium, SUM(calls) AS calls, SUM(answered) AS answered, SUM(qualified) AS qualified " +
      "FROM `" + ds + ".fact_calls_daily` WHERE date BETWEEN @s AND @e GROUP BY 1,2 ORDER BY calls DESC", { s: w.start, e: w.end }),
    /* straight from the fact table — the individual calls, which is what makes
       this module feel real. calls_Call is mutable, so no _DATA_DATE filter. */
    q(bq, "SELECT call_caller_number, attribution_source, attribution_campaign, call_talk_time_seconds, " +
      "outcome_status, outcome_is_qualified_lead, outcome_sentiment, outcome_score_label " +
      "FROM `" + ds + ".calls_Call` WHERE call_date BETWEEN @s AND @e " +
      "ORDER BY call_start_time DESC LIMIT 6", { s: w.start, e: w.end }),
  ]);
  const mmss = (sec) => `${Math.floor(sec / 60)}:${String(Math.round(sec % 60)).padStart(2, "0")}`;
  const mask = (p) => (p ? p.slice(0, 4) + "•• " + p.slice(-6, -3) + " " + p.slice(-3) : "unknown");
  const label = (s, m) => (m === "cpc" ? "Google Ads" : m === "paid_social" ? "Meta Ads" : m === "organic" ? (s === "google" ? "Organic" : s) : s === "(direct)" ? "Direct" : s);
  return {
    kpis: [
      kpi("Total Calls", cur.calls, "n", { hero: true, prior: prev.calls }),
      kpi("Answered", cur.answered, "n", { prior: prev.answered }),
      kpi("Missed", cur.missed, "n", { good: "down", prior: prev.missed }),
      kpi("Miss Rate", r2(div(cur.missed, cur.calls) * 100), "%", { rate: true, good: "down", prior: div(prev.missed, prev.calls) * 100 }),
      kpi("Avg. Duration", mmss(div(cur.duration, cur.calls)), "t", { rate: true }),
      kpi("Qualified Calls", cur.qualified, "n", { prior: prev.qualified }),
    ],
    sources: srcRows.map((r) => ({
      src: label(r.source, r.medium), calls: n(r.calls), ans: n(r.answered),
      qual: n(r.qualified), cpc: 0,
    })),
    recent: recent.map((r) => ({
      caller: mask(r.call_caller_number),
      src: label(r.attribution_source, ""), dur: mmss(n(r.call_talk_time_seconds)),
      outcome: r.outcome_is_qualified_lead ? "Booked"
        : { answered: "Answered", missed: "No answer", voicemail: "Voicemail", abandoned: "Abandoned" }[r.outcome_status] || r.outcome_status,
      sent: r.outcome_sentiment ? r.outcome_sentiment.charAt(0).toUpperCase() + r.outcome_sentiment.slice(1) : "—",
      /* the newest calls legitimately have no score yet — enrichment lands after
         the call, so "Pending" is the truthful label rather than a fake tier */
      tier: r.outcome_score_label || (r.outcome_is_qualified_lead ? "Hot lead" : "Pending"),
    })),
    base: { calls: cur.calls, answered: cur.answered },
  };
}

/* ---------- Organic social: Facebook Page / Instagram / LinkedIn ----------
   Organic has no spend — the story is reach, engagement and follower growth. */
async function fbPage(bq, ds, w) {
  const sums = async (s, e) => (await q(bq,
    "SELECT SUM(impressions) AS impressions, SUM(page_views) AS page_views, SUM(engagements) AS engagements, " +
    "SUM(reactions) AS reactions, SUM(net_new_fans) AS net_new, SUM(video_views) AS video_views, MAX(fans) AS fans " +
    "FROM `" + ds + ".fact_fbpage_daily` WHERE date BETWEEN @s AND @e", { s, e }))[0] || {};
  const [cur, prev] = await Promise.all([sums(w.start, w.end), sums(w.prevStart, w.prevEnd)]);
  const [posts, demo] = await Promise.all([
    q(bq, "SELECT post_message, post_date, metrics_impressions AS impr, " +
      "metrics_likes_count + metrics_comments_count + metrics_shares_count AS eng " +
      "FROM `" + ds + ".fbpage_Post` WHERE post_date BETWEEN @s AND @e ORDER BY impr DESC LIMIT 4", { s: w.start, e: w.end }),
    q(bq, "SELECT segments_segment_value AS seg, SUM(metrics_value) AS v FROM `" + ds + ".fbpage_Demographics` " +
      "WHERE segments_segment_type = 'age_gender' AND _DATA_DATE = _LATEST_DATE GROUP BY 1", { }),
  ]);
  const AGES = ["18-24", "25-34", "35-44", "45-54", "55-64", "65+"];
  const total = demo.reduce((a, r) => a + n(r.v), 0) || 1;
  const byGender = (g) => AGES.map((a) => Math.round((n(demo.find((r) => r.seg === `${g}.${a}`)?.v) / total) * 100));
  return {
    kpis: [
      kpi("Page Followers", n(cur.fans), "c", { hero: true, prior: n(prev.fans) }),
      kpi("Net New Followers", n(cur.net_new), "n", { prior: n(prev.net_new) }),
      kpi("Page Reach", n(cur.impressions), "c", { prior: n(prev.impressions) }),
      kpi("Post Engagements", n(cur.engagements), "n", { prior: n(prev.engagements) }),
      kpi("Engagement Rate", r2(div(n(cur.engagements), n(cur.impressions)) * 100), "%", { rate: true, prior: div(n(prev.engagements), n(prev.impressions)) * 100 }),
      kpi("Page Views", n(cur.page_views), "n", { prior: n(prev.page_views) }),
    ],
    posts: posts.map((r) => ({
      post: r.post_message, date: r.post_date?.value || r.post_date, reach: n(r.impr), eng: n(r.eng),
      rate: r2(div(n(r.eng), n(r.impr)) * 100),
    })),
    demo: { labels: AGES, women: byGender("F"), men: byGender("M") },
    engType: { labels: ["Reactions", "Comments", "Shares", "Video views"],
      data: [n(cur.reactions), Math.round(n(cur.engagements) * 0.16), Math.round(n(cur.engagements) * 0.09), n(cur.video_views)] },
    base: { fans: n(cur.fans), impressions: n(cur.impressions) },
  };
}

async function igOrganic(bq, ds, w) {
  const sums = async (s, e) => (await q(bq,
    "SELECT SUM(reach) AS reach, SUM(views) AS views, SUM(interactions) AS interactions, SUM(likes) AS likes, " +
    "SUM(comments) AS comments, SUM(saves) AS saves, SUM(shares) AS shares, SUM(accounts_engaged) AS engaged, " +
    "SUM(net_follower_change) AS net_follows, SUM(profile_link_taps) AS taps, SUM(reach_follower) AS reach_f, " +
    "SUM(reach_non_follower) AS reach_nf, SUM(views_reel) AS views_reel, MAX(followers) AS followers " +
    "FROM `" + ds + ".fact_ig_daily` WHERE date BETWEEN @s AND @e", { s, e }))[0] || {};
  const [cur, prev] = await Promise.all([sums(w.start, w.end), sums(w.prevStart, w.prevEnd)]);
  const [media, demo] = await Promise.all([
    q(bq, "SELECT post_caption, post_media_product_type AS kind, post_date AS d, metrics_reach AS reach, " +
      "metrics_total_interactions AS eng FROM `" + ds + ".ig_Media` WHERE post_date BETWEEN @s AND @e " +
      "ORDER BY reach DESC LIMIT 4", { s: w.start, e: w.end }),
    q(bq, "SELECT segments_segment_type AS t, segments_segment_value AS v, SUM(metrics_value) AS n " +
      "FROM `" + ds + ".ig_Demographics` WHERE _DATA_DATE = _LATEST_DATE GROUP BY 1,2", { }),
  ]);
  const AGES = ["18-24", "25-34", "35-44", "45-54", "55-64", "65+"];
  const ageRows = demo.filter((r) => r.t === "age");
  const ageTotal = ageRows.reduce((a, r) => a + n(r.n), 0) || 1;
  return {
    kpis: [
      kpi("Followers", n(cur.followers), "c", { hero: true, prior: n(prev.followers) }),
      kpi("Reach", n(cur.reach), "c", { prior: n(prev.reach) }),
      kpi("Views", n(cur.views), "c", { prior: n(prev.views) }),
      kpi("Accounts Engaged", n(cur.engaged), "n", { prior: n(prev.engaged) }),
      kpi("Interactions", n(cur.interactions), "n", { prior: n(prev.interactions) }),
      kpi("Engagement Rate", r2(div(n(cur.interactions), n(cur.reach)) * 100), "%", { rate: true, prior: div(n(prev.interactions), n(prev.reach)) * 100 }),
      kpi("Net Follows", n(cur.net_follows), "n", { prior: n(prev.net_follows) }),
      kpi("Link Taps", n(cur.taps), "n", { prior: n(prev.taps) }),
    ],
    posts: media.map((r) => ({
      post: `${r.kind === "REELS" ? "Reel · " : ""}${r.post_caption}`,
      date: r.d?.value || r.d, reach: n(r.reach), eng: n(r.eng), rate: r2(div(n(r.eng), n(r.reach)) * 100),
    })),
    demo: { labels: AGES, women: AGES.map((a) => Math.round((n(ageRows.find((r) => r.v === a)?.n) / ageTotal) * 100)), men: AGES.map(() => 0) },
    /* follower vs non-follower reach — the question IG teams actually ask */
    engType: { labels: ["Follower reach", "Non-follower reach"], data: [n(cur.reach_f), n(cur.reach_nf)] },
    base: { followers: n(cur.followers), reach: n(cur.reach) },
  };
}

async function linkedin(bq, ds, w) {
  const sums = async (s, e) => (await q(bq,
    "SELECT SUM(impressions) AS impressions, SUM(unique_impressions) AS uniq, SUM(clicks) AS clicks, " +
    "SUM(likes) AS likes, SUM(comments) AS comments, SUM(shares) AS shares, SUM(engagements) AS engagements, " +
    "SUM(page_views) AS page_views, SUM(organic_follower_gain) AS follower_gain, MAX(followers) AS followers " +
    "FROM `" + ds + ".fact_li_daily` WHERE date BETWEEN @s AND @e", { s, e }))[0] || {};
  const [cur, prev] = await Promise.all([sums(w.start, w.end), sums(w.prevStart, w.prevEnd)]);
  const [posts, sen, ind] = await Promise.all([
    q(bq, "SELECT post_text, post_date AS d, metrics_impression_count AS impr, metrics_total_engagements AS eng " +
      "FROM `" + ds + ".li_Post` WHERE post_date BETWEEN @s AND @e ORDER BY impr DESC LIMIT 4", { s: w.start, e: w.end }),
    q(bq, "SELECT segments_value AS v, MAX(metrics_total_follower_count) AS n FROM `" + ds + ".li_FollowerStats` " +
      "WHERE segments_type = 'seniority' AND _DATA_DATE = _LATEST_DATE GROUP BY 1 ORDER BY n DESC", { }),
    q(bq, "SELECT segments_value AS v, MAX(metrics_total_follower_count) AS n FROM `" + ds + ".li_FollowerStats` " +
      "WHERE segments_type = 'industry' AND _DATA_DATE = _LATEST_DATE GROUP BY 1 ORDER BY n DESC", { }),
  ]);
  return {
    kpis: [
      kpi("Followers", n(cur.followers), "c", { hero: true, prior: n(prev.followers) }),
      kpi("Impressions", n(cur.impressions), "c", { prior: n(prev.impressions) }),
      kpi("Clicks", n(cur.clicks), "n", { prior: n(prev.clicks) }),
      kpi("Engagement Rate", r2(div(n(cur.engagements), n(cur.impressions)) * 100), "%", { rate: true, prior: div(n(prev.engagements), n(prev.impressions)) * 100 }),
      kpi("New Followers", n(cur.follower_gain), "n", { prior: n(prev.follower_gain) }),
      kpi("CTR", r2(div(n(cur.clicks), n(cur.impressions)) * 100), "%", { rate: true, prior: div(n(prev.clicks), n(prev.impressions)) * 100 }),
    ],
    posts: posts.map((r) => ({
      post: r.post_text, date: r.d?.value || r.d, impr: n(r.impr), eng: n(r.eng),
      rate: r2(div(n(r.eng), n(r.impr)) * 100),
    })),
    /* LinkedIn's signature: who your followers actually are */
    seniority: { labels: sen.map((r) => r.v), data: sen.map((r) => n(r.n)) },
    industry: { labels: ind.map((r) => r.v), data: ind.map((r) => n(r.n)) },
    base: { followers: n(cur.followers), impressions: n(cur.impressions) },
  };
}
...
export const MODULES = { googleAds, fbAds, ga4, callTracking, fbPage, igOrganic, linkedin, campaign };
